...
function addImportStatementToCurrentFile(item: QuickPickItem, config: Config) {
    ...
    const statementES6 = config.ES6Destructuring ? `import {} from ${quotes}${item.label}${quotes}${config.importLinebreak}` : `import ${item.label.replace(/-/g, "")} from ${quotes}${item.label}${quotes}${config.importLinebreak}`;
